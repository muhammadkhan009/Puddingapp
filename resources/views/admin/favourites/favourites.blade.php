        ...
        <div class="row">
          <div class="col-lg-12">
            <div class="card">
                ...
                <h3>Favourite Products({{count($products)}})</h3>
                    <a href="{{route('add-favourites')}}" class="btn btn-xl btn-primary">Add Favourite Product</a>
                </div>
                @csrf
                    <table id="zipscodes" class="table table-bordered table-hover" >
                        ...
                        <tr>
                            <td>{{$loop->index+1}}</td>
                            <td>{{$product->name}}</td>
                            <td style="text-align:center;"><img src="{{asset('public/uploads/favourite-products').'/'.$product->product_image}}" height="50" /></td>
                            ...
                            @endif </td> -->
                            <td><a href="{{route('edit-favourite-product', ['id'=>$product->id])}}" class="btn btn-block btn-primary">EDIT</a></td>
  ...
  $(document).ready( function () {
   $("#zipscodes").DataTable({
    ...
    $('.btn-danger').on('click', function(){
      ...
      if(response == true){
        $.ajax({
                url:        "{{route('ajax.favourite.product.del.req')}}",
                type:       'POST',
